...
extern void enter()
{
	...
	if (fopen_s(&f, "Bilety.txt", "w+") == 0)
	{
		...
		{
			fprintf(f, "\nНомер операции: %d\nСезон: %s\nКол-во: %d\nЦена за 1: %0.2f\nИтого: %d\n", 
				(*(data + a)).nomer, (*(data + a)).season, (*(data + a)).kolvo, (*(data + a)).price, (*(data + a)).itog_stoimost);
		}
		printf("\n");
...
extern void print()
{
	...
	else
	{
		...
		////{
		//
		//	while (fscanf_s(f, "%d%s%d%f%d", (*(data + a)).nomer, (*(data + a)).season, (*(data + a)).kolvo,
		//		(*(data + a)).price, (*(data + a)).itog_stoimost) != EOF)
		//	//while (fscanf_s(f, "%d %s %d %f %d", &(data[a].nomer), data[a].season, &(data[a].kolvo),
		//	//	&(data[a].price), &(data[a].itog_stoimost)) != EOF)
		//	//while(!feof(f))
		//	{
				...
				if (nom == (*(data + a)).nomer)						//проверка наличия записей с введённым номером
				{
					...
					do {
						...
						if (fopen_s(&f, "Bilety.txt", "w+") == 0)
						{
							...
							for (a = 0; a < kol_biletov; a++)			//Ввод информации в файл
							{
								fprintf(f, "\nНомер операции: %d\nСезон: %s\nКол-во: %d\nЦена за 1: %0.2f\nИтого: %d\n",
									(*(data + a)).nomer, (*(data + a)).season, (*(data + a)).kolvo, (*(data + a)).price, (*(data + a)).itog_stoimost);
							}
...
extern void del()
{
	...
	if (kol_biletov <= 0)										//проверка наличия записей
	{
		printf("\nОтсутствуют записи для удаления.\n");
	}
	else
	{
		...
		for (a = 0; a < kol_biletov; a++)
		{
			if (d == (*(data + a)).nomer)						//проверка наличия записей с введённым номером
			{
...
extern void sort_nomer()
{
	...
	if (fopen_s(&f, "Bilety.txt", "w+") == 0)		//Ввод информации в файл
	{
...
//		for (a = 0; a < kol_biletov; a++)
//		{
//			printf("%d %12s %12d %18.2f %22.2f", (*(data + a)).nomer, (*(data + a)).season, (*(data + a)).kolvo, (*(data + a)).price, (*(data + a)).itog_stoimost);
//			printf("\n");
//		}
//		printf("\n");
//	}
